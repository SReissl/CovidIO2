runModel<-function(IO,measures,cons_coeff,params,rounds,LD,LD2,scenario,ExpService,ExpSectoral,mobility,nodemand1,nodemand2){
...
Tax=(IO[674,(n+1):(ncol(IO))])
Taxes_regions=c(rep(0,20))
for(i in 1:(nItalianRegions-1)){
  Taxes_regions[i]<-sum(Tax[((i-1)*16+1):((i-1)*16+16)])
}
...
outputShocksSectors=array(0,dim=c(nbRounds,32))
outputShocksRegions=array(0,dim=c(nbRounds,20))

ratioShocks=c(rep(0,nbRounds))
ratioShocksSectors=array(0,dim=c(nbRounds,32))
...
demandSectors=array(0,dim=c(nbRounds,32))
demandRegions=array(0,dim=c(nbRounds,20))
producibleTotal=c(rep(0,nbRounds))
producibleSectors=array(0,dim=c(nbRounds,32))
producibleRegions=array(0,dim=c(nbRounds,20))
consInfTotal=c(rep(0,nbRounds))
consHTotal=c(rep(0,nbRounds))
consInfRegions=array(0,dim=c(nbRounds,20))
consHRegions=array(0,dim=c(nbRounds,20))
consInfSectors=array(0,dim=c(nbRounds,32))
consHSectors=array(0,dim=c(nbRounds,32))
...
ratioSectors=array(0,dim=c(nbRounds,32))
ratioDemandSectors=array(0,dim=c(nbRounds,32))
baselineOutput=rowSums(IO[1:672,])
...
productionShock[1:((nItalianRegions-1)*nSectors),54]=-(productionShock[1:((nItalianRegions-1)*nSectors),46]+productionShock[1:((nItalianRegions-1)*nSectors),49]+productionShock[1:((nItalianRegions-1)*nSectors),52])
}
...
maxOutputFeasible=pmin((laborAvailable/laborLeontiefCoefficients),colMins(maxOutputGivenIntermediateInputs))
        ...
        for(i in 1:nrow(coefficienti_consumo)){
          ...
          for(j in 1:nItalianRegions){
    		...
    		consInfTotal[t]=sum(cons_regions_inf)
    		consHTotal[t]=sum(cons_regions)
    		ratioDemand[t,]<-rowSums(cons_regions_inf)/rowSums(cons_regions)
    		ratioDemandTotal[t]<-sum(ratioDemand[t,]*baselineOutput/sum(baselineOutput),na.rm = TRUE)
    		
    		for(i in 1:(nItalianRegions-1)){
    		  consInfRegions[t,i]<-sum(cons_regions_inf[((i-1)*nSectors+1):((i-1)*nSectors+nSectors),])
    		  consHRegions[t,i]<-sum(cons_regions[((i-1)*nSectors+1):((i-1)*nSectors+nSectors),])
    		  ratioDemandRegions[t,i]<-sum(baselineOutput[((i-1)*nSectors+1):((i-1)*nSectors+nSectors)]/sum(baselineOutput[((i-1)*nSectors+1):((i-1)*nSectors+nSectors)])*ratioDemand[t,((i-1)*nSectors+1):((i-1)*nSectors+nSectors)],na.rm = TRUE)
    	  }
    		for(i in 1:nSectors){
    		  consInfSectors[t,i]<-sum(cons_regions_inf[((0:(nItalianRegions-1))*nSectors+i),])
    		  consHSectors[t,i]<-sum(cons_regions[((0:(nItalianRegions-1))*nSectors+i),])
    		  ratioDemandSectors[t,i]<-sum(baselineOutput[((0:(nItalianRegions-1))*nSectors+i)]/sum(baselineOutput[((0:(nItalianRegions-1))*nSectors+i)])*ratioDemand[t,((0:(nItalianRegions-1))*nSectors+i)],na.rm = TRUE)
    		}
        ...
        maxOutputGivenIntermediateInputs=availableInputs2/A
    		maxOutputGivenIntermediateInputs[is.nan(maxOutputGivenIntermediateInputs)]=Inf
        ...
        #what is the ratio between capacity of production and demand?
    		...
    		for(i in 1:(nItalianRegions-1)){
    		  GDP_regions[t,i]=sum(fdDelivered[((i-1)*nSectors+1):((i-1)*nSectors+nSectors),1])+sum(changeInventories[,((i-1)*32+1):((i-1)*32+32)])-sum(importsDelivered[((i-1)*32+1):((i-1)*32+32)])+sum(changeImports[((i-1)*32+1):((i-1)*32+32)])+Taxes_regions[i]
    		}
...
prod_sectors_m[13,]=colSums(prod_sectors[54:57,])
prod_sectors_m[14,]=colSums(prod_sectors[58:61,])
prod_sectors_m[15,]=colSums(prod_sectors[62:65,])
prod_sectors_m[16,]=colSums(prod_sectors[66:70,])
prod_sectors_m[17,]=colSums(prod_sectors[71:74,])
prod_sectors_m[18,]=colSums(prod_sectors[75:78,])
...
prod_sectors_q[5,]=colSums(prod_sectors_m[13:15,17:20])/12*13
prod_sectors_q[6,]=colSums(prod_sectors_m[16:18,17:20])
...
lossProd=(colSums(prod_sectors_m[1:12,])-1200)/1200
...
expenditure_q[5,]=colSums(expenditure_m[13:15,])/12*13
expenditure_q[6,]=colSums(expenditure_m[16:18,])
...
lossExpenditure=(colSums(expenditure_q_3[1:4,])-400)/400
lossServices=(colSums(prod_sectors_q[1:4,])-400)/400
...
GDP_mregions[,1]=rowSums(GDP_regions[,NO])
GDP_mregions[,2]=rowSums(GDP_regions[,NE])
GDP_mregions[,3]=rowSums(GDP_regions[,CE])
GDP_mregions[,4]=rowSums(GDP_regions[,MG])
...
GDP_qr=array(data=0,dim=c(6,(nItalianRegions-1)))
GDP_qr[1,]<-colSums(GDP_regions[1:13,])
GDP_qr[2,]<-colSums(GDP_regions[14:26,])
GDP_qr[3,]<-colSums(GDP_regions[27:39,])
GDP_qr[4,]<-colSums(GDP_regions[40:53,])
GDP_qr[5,]<-colSums(GDP_regions[54:65,])
GDP_qr[6,]<-colSums(GDP_regions[66:78,])
GDP_qr[4,]<-GDP_qr[4,]*13/14
GDP_qr[5,]<-GDP_qr[5,]*13/12


GDP_qmr=array(data=0,dim=c(6,4))
GDP_qmr[1,]<-colSums(GDP_mregions[1:13,])
GDP_qmr[2,]<-colSums(GDP_mregions[14:26,])
GDP_qmr[3,]<-colSums(GDP_mregions[27:39,])
GDP_qmr[4,]<-colSums(GDP_mregions[40:53,])
GDP_qmr[5,]<-colSums(GDP_mregions[54:65,])
GDP_qmr[6,]<-colSums(GDP_mregions[66:78,])
GDP_qmr[4,]<-GDP_qmr[4,]*13/14
GDP_qmr[5,]<-GDP_qmr[5,]*13/12

GDP_q=GDP_q/(GDP[1]*13)*100
for(i in 1:(nItalianRegions-1)){
GDP_qr[,i]=GDP_qr[,i]/(GDP_regions[1,i]*13)*100
}
for(i in 1:4){
GDP_qmr[,i]=GDP_qmr[,i]/(GDP_mregions[1,i]*13)*100
}


simvec=c(lossProd[1:16],lossServices,lossExpenditure)

return(list(simvec,prod_sectors_m[,1:16],prod_sectors_q,expenditure_q_3,GDP_q,expenditure_3_absolute,GDP,Cons,Inv,sales,sales_sectors,intensity11,intensity12,intensity21,intensity22,bottlenecks,delivered,capacitySectors,capacityRegions,GDP_qr,GDP_qmr,demandTotal,demandRegions,demandSectors,producibleTotal,producibleRegions,producibleSectors,consInfTotal,consHTotal,consInfRegions,consHRegions,consInfSectors,consHSectors,ratioTotal,ratioDemandTotal,ratioRegions,ratioDemandRegions,ratioSectors,ratioDemandSectors,GDPShocks,outputShocks,outputShocksRegions,outputShocksSectors,ratioShocks,ratioShocksRegions,ratioShocksSectors,outputTotal))
